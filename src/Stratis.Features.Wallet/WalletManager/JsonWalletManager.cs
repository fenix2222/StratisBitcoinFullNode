...
{
    /// <summary>
    /// A manager providing operations on wallets stored as single JSON document file per wallet.
    /// </summary>
    public class JsonWalletManager : BaseWalletManager
    {
        ...
        public ConcurrentBag<IWallet> Wallets { get; }
            ...
            IBroadcasterManager broadcasterManager = null) // no need to know about transactions the node will broadcast to.
        {
            ...
            this.Wallets = new ConcurrentBag<IWallet>();
        ...
        public void Start()
        {
            ...
            IEnumerable<IWallet> wallets = this.fileStorage.LoadByFileExtension(WalletFileExtension);
        ...
        /// <inheritdoc />
        public IWallet LoadWallet(string password, string name)
        {
            Guard.NotEmpty(password, nameof(password));
            Guard.NotEmpty(name, nameof(name));

            // Load the file from the local system.
            IWallet wallet = this.fileStorage.LoadByFileName($"{name}.{WalletFileExtension}");
        ...
        /// <inheritdoc />
        public void SaveWallet(IWallet wallet)
        {
            Guard.NotNull(wallet, nameof(wallet));
            this.SaveWalletInternal(wallet);
        }

        protected virtual void SaveWalletInternal(IWallet wallet)
        {
            if (wallet is Wallet walletInstance)
            {
                lock (this.lockObject)
                {
                    this.fileStorage.SaveToFile(walletInstance, $"{wallet.Name}.{WalletFileExtension}", new FileStorageOption { SerializeNullValues = false });
                }
            }
            else
            {
                throw new WalletException($"You can save only wallets of type Wallet in using {nameof(JsonWalletManager)}");
            }
        }
        ...
        private void Load(IWallet wallet)
        {
            Guard.NotNull(wallet, nameof(wallet));
